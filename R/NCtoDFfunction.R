...
NC2DFR <- function(PATH){
  ...
  for(i in 1:length(listfilesregional)){
    ...
    varreg_pre <- varreg_pre %>% mutate(Time=ymd_hms(as.character(Time))) %>%
      ...
      group_by(Year,Month,xc,yc) %>% summarise(m=mean(c1)) %>% ungroup()  ##Convierte la variable time en tres variables que indican el año y mes y saca la media de las observaciones para ese mes.
