#' VisualizeDATA function generates a map of the chosen variable in a specific month and year
#'
...
#' @examples
#' @import dplyr stringr  ncdf4 lubridate reshape2 sp ggmap
#'
#'
#'

VisualizeDATA <- function(datos,var,year, month){
  lat <- c(min(datos$lat),max(datos$lat))
  long <- c(min(datos$lon),max(datos$lon))-360
  bbox <- make_bbox(long,lat,f=0.05)
  b <- get_map(bbox,maptype="toner-lite",source="stamen", color="bw")
  ...
  mes <- c("enero","febrero","marzo","abril",
           ...
           "setiembre","agosto","octubre","noviembre","diciembre")
  png(filename="Gr6.png")
  G6 <- ggmap(b) + geom_point(data = datos[datos$Year==YY&datos$Month==MM,],
                              aes(lon-360,lat,color=variable),alpha=0.9) +
